...
def runmodel():
    ...
    params_dissolved = filter(lambda p: p in PGRP['dissolved'], RUN['p'])
    params_particulate = filter(lambda p: p in PGRP['particulate'], RUN['p'])
...
def rungis2tbls():
    ...
    excldvarset = set(INDOC.keys() + ['BasinID', 'BasinArea'])
